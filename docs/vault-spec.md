...
| Exposure of credential *contents* at rest (usernames/passwords/notes/TOTP are AES-256-GCM encrypted; only ciphertext is on disk). | **Metadata leak from the raw DB file.** Item titles and associated hosts are stored as cleartext metadata (§6), so an attacker with the database file but **not** the vault key can still learn *which sites you have entries for* — just not the credentials themselves. Imported **non-login** items (secure notes / cards / addresses) keep the same shape — their bodies (card numbers, note text) live in the **encrypted payload**; only `title`/`host` are cleartext. Recognizing them as non-login (host `sn` → `type = secureNote`, §6/§10) changes only a cleartext `type` flag. **Payment cleartext hint (TV-2b):** a card's **last-4 + detected network** (`items.last4` / `card_network`) are stored in the clear so the Payment list row renders without decrypting — a raw-DB attacker (without the vault key) also learns each card's last-4 + network. This is **low-sensitivity** (not the full PAN, not the CVV, not enough to transact — last-4 is the standard displayed identifier, the network is public); the **full number + CVV stay encrypted-payload-only**. **Bank-account cleartext hint (TV-2c):** likewise a bank account's **last-4 of the account number** (`items.account_last4`) is stored in the clear so the Bank list row renders without decrypting — a raw-DB attacker also learns each account's last-4. Same low-sensitivity rationale (last-4 only); the **full account number, IBAN, and PIN stay encrypted-payload-only**. Nothing beyond these last-4 / network hints is stored in the clear. |
