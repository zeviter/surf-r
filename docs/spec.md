...
- **Typed vault TV-1 + TV-2a done** (`docs/typed-vault-wireframes.md`): four item types (login · payment ·
  ...
  the **segmented vault list, type picker, and Secure Note + Address detail/edit/copy** with the inherited
  ESC/back nav (TV-2a). The zero-decryption list invariant holds across all four segments. **Remaining:**
  **TV-2b** (Payment detail/edit — masking + biometric reveal + card-type detection) and **TV-3**
  (card/address web-fill, pairs with Slice 10).
