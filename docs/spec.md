...
- **Typed vault TV-1 done** (`docs/typed-vault-wireframes.md`): four item types (login · payment · address ·
  secure note) under the same AES-256-GCM envelope (no crypto change); pure `TypedNoteParser` for LastPass
  `NoteType:Credit Card`/`Address` classification + lossless field extraction; one-time re-classification of
  imported notes. Headless — **TV-2** (per-type UI) and **TV-3** (card/address web-fill, pairs with Slice 10)
  remain.
